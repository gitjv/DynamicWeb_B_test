...
function indiLinked(idx, citations, dates)  // JV added a dates option
{
	...
	dates = (typeof(dates) !== 'undefined') ? dates : true; // JV if no dates opt then assume it is needed
	var txt = I(idx, 'name')
	txt += gidBadge(I(idx, 'gid'));
	if (dates) txt += indiDates(I(idx, 'birth_date'), I(idx, 'death_date'));  // JV if test
...
function indiDetails(idx)
{
	...
	if (I(idx, 'life_span') != '') txt += '<tr><td class="dwr-attr-title">' + _('Life span') + '</td><td colspan="2" class="dwr-attr-value">' + I(idx, 'life_span') + '</td></tr>'; // JV
	if (I(idx, 'why_me') != '') txt += '<tr><td class="dwr-attr-title">' + _('Why me') + '</td><td colspan="2" class="dwr-attr-value">' + I(idx, 'why_me') + '</td></tr>'; // JV
	if (I(idx, 'track_relationships') != '') txt += trackRels(I(idx, 'track_relationships')); // JV
	if (I(idx, 'story') != '') txt += '<tr><td class="dwr-attr-title">' + _('Story') + '</td><td colspan="2" class="dwr-attr-value">' + I(idx, 'story') + '</td></tr>'; // JV
	txt += '</table>';
	return(txt);
}


function trackRels(track_relationships)
// put track relationships into a table structure JV
{
	var txt = '';
	txt += '<tr><td rowspan="' + track_relationships.length +'" class="dwr-attr-title">' + _('Tracking') + '</td>';
	var rel_text = '';
	var rel_index = '';
	var relative = '';
	rel_text = track_relationships[0][0];
	rel_index = track_relationships[0][3];
	relative = indiLinked(rel_index);
	// the first track row continues its heading row, so it's markup is a bit different
	txt += '<td colspan="2" class="dwr-attr-value">' + rel_text + ' ' + _('of') +' ' + relative + '</td></tr>';
	var j;
	for (j = 1; j < track_relationships.length; j++)
	{
		rel_text = track_relationships[j][0];
		rel_index = track_relationships[j][3];
		relative = indiLinked(rel_index, false);
		txt += '<tr><td colspan="2" class="dwr-attr-value">' + rel_text + ' ' + _('of') + ' ' + relative + '</td></tr>';
	}
	return(txt);
}
...
function eventTable(events, idx, fdx, extended=false)
// JV added 'extended' parameter for extended event capability
// and behaviours for extended events table
{
	...
	if (extended) {
		contents.title = _('Events');  // JV
		//txt+= '<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>';

		if (EXTENDED_EVENTS_NOTE !== ''){  // insert explanatory note
			txt += PrintTitle('EE', 5,
						[{
							title: _('About...'),
							text: EXTENDED_EVENTS_NOTE
						}])
		}

		// JV checkboxes for vitals - (births deaths)
		// puts all checkboxes in one div container, lifespan in a  sub div with marginleft auto, to push it right
		txt += '<div id="checkbox-container" style="display: flex; ">';
		txt += _('Show vital events for:') + '&ensp;';
		txt += `<div>
    			<input type="checkbox" id="vital_child" onclick="vital_clicked(\'child\')" checked>
	   			<label for="vital_child"> ${ _("Children")} </label>&emsp;
	  			</div>`;
		txt += `<div>
    			<input type="checkbox" id="vital_spouse" onclick="vital_clicked(\'spouse\')" checked>
	   			<label for="vital_spouse"> ${ _("Spouses")} </label>&emsp;
	  			</div>`;
		txt += `<div>
    			<input type="checkbox" id="vital_sibling" onclick="vital_clicked(\'sibling\')" checked>
	   			<label for="vital_sibling"> ${ _("Siblings")} </label>&emsp;
	  			</div>`;
		txt += `<div>
    			<input type="checkbox" id="vital_parent" onclick="vital_clicked(\'parent\')" checked>
	   			<label for="vital_parent"> ${ _("Parents")} </label>&emsp;
	  			</div>`;
		txt += `<div style="margin-left: auto;">
    			<input type="checkbox" id="lifespan" onclick="lifespan_clicked()">
	   			<label for="lifespan"> ${ _("Limit events to lifespan")} </label>
	  			</div>`;
		txt += ' </div>';

		txt += '<table id="extended_events" class="table table-condensed table-bordered dwr-table-panel">';
	}
	else {  // ie  the original events table
		contents.title = _("Events tab");
		txt += '<table id="events" class="table table-condensed table-bordered dwr-table-panel">';
	}

	txt += '<div id="hdrcheckboxes" ><thead ><tr>';
	if (extended) {
		txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('Age') + '</th>';
		txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('In lifespan') + '</th>';
		txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('Kind') + '</th>';
		txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('date sdn') + '</th>';
		txt += "<script>$('#extended_events tr > *:nth-child(2)').hide();</script>";  // but hide in_lifespan (col 2)
		txt += "<script>$('#extended_events tr > *:nth-child(3)').hide();</script>";  // but hide kind (col 3)
		txt += "<script>$('#extended_events tr > *:nth-child(4)').hide();</script>";  // but hide date sdn (col 4)
	}
	txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('Event') + '</th>';
	txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('Date') + '</th>';
	if (extended) txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('Who') + '</th>';  // JV
	txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('Place') + '</th>';

	if (extended) {
		txt += '<th class="dwr-attr-header" id="notesid" style="vertical-align: sub">'  +
			'<input type="checkbox" id="EEnoteCheck" onclick="EEnote_clicked()" checked>' +
			'<label for="EEnoteCheck">&ensp;' +  _("Notes")+ '</label></th>';
	} else {
		txt += '<th class="dwr-attr-header" style="vertical-align: sub">' + _('Notes') + '</th>';
	}

	txt += '</tr></thead></div><tbody>';
	...
	for (var j = 0; j < events.length; j++)
	{
		...
		txt += '<tr>';
		if (extended) {
			txt += '<td class="dwr-attr-value">' + e.age + '</td>';
			txt += '<td class="dwr-attr-value">' + (e.in_lifespan) + '</td>';
			txt += '<td class="dwr-attr-value">' + (e.kind) + '</td>';
			txt += '<td class="dwr-attr-value">' + (e.date_sdn) + '</td>';
		}
		...
		if (extended) {  // JV
			if (e.who_in_family !== ''){
				txt += '<td class="dwr-attr-value">';
				// if (e.pc_reln) {if (e.pc_reln !== 'Birth') txt += '(' + e.pc_reln  + ')' + '<br>'};
				// txt += indiLinked(e.who_in_family, false,false) + '</td>';
				// changed order of who and relnship
				txt += indiLinked(e.who_in_family, false,false) + '<br>';
				if (e.pc_reln) {if (e.pc_reln !== 'Birth') txt +=  '<small>' +'(' + e.pc_reln  + ')' + '</small>'+ '</td>'};

			}
			else{
				txt += '<td class="dwr-attr-value">'+ '</td>';
			}
		}
		...
		{
			var p_idx = e.part_person[i][0];  // now a list of lists to allow for person and role as well
			// don't show current person as participant, and don't show 'who' as particpant
			if (p_idx != idx && p_idx != e.who_in_family) participants += '<br>' + indiLinked(p_idx, false) + '<small>' + ' ' + e.part_person[i][1] + '</small>';
		}
		for (var i = 0; i < e.part_family.length; i += 1)
		{
			var p_fdx = e.part_family[i];
			// if (p_fdx != fdx) participants += '<br>' + indiLinked(p_fdx, false);  // JV I think this is a bug, should be a famLinked ref
			if (p_fdx != fdx) participants += '<br>' + famLinked(p_fdx, false);
		}
...
// }
//
// function lifespan_clicked() {  // JV when lifespan clicked, can just redo the vital calls, as they handle lifespan limit
//     vital_clicked("child");
...
// }
//
// function EEnote_clicked() {  // JV show or hide notes data (but keep the heading)
...
// }
//
// var initial_dwrvalues = {  // JV to define initial vital tick values if they don't exist in local storage yet.
//     vital_child: true,
//     vital_spouse: true,
//     vital_sibling: true,
//     vital_parent: true,
...
// }
//
// function get_dwrstorage(key) {  // JV get the localstorage called key, set to initial if not found
...
function printIndi(idx)
{
	...
	var evtabs;  // JV include  extended, or old plus extended tabs
	if (SHOW_OLD_EVENTS_TAB) {
		evtabs = [].concat(eventTable(I(idx, 'events'), idx, -1, false),
			eventTable(I(idx, 'extended_events'), idx, -1, true));
	}
	else{
		evtabs = [].concat(eventTable(I(idx, 'extended_events'), idx, -1, true));
	}
...
function printIndiAncestors(idx)
{
	...
	for (var j = 0; j < famc_list.length; j++)
	{
		var fdx = famc_list[j];
		var jvalue = j
		html += PrintTitle('Idsc' + j + idx, 4,
			[{
				// JV added union type to title AND placed siblings box inside the family box
				// AND fixed collapsing
				title: _('Parents') + ': ' + famLinked(fdx) + ' <small> ' + _('Union:') + ' ' + F(fdx, 'type') + '</small>',
				text: printIndiParents(fdx, idx, jvalue) //now has extra parameters JV

					+ PrintTitle('IdscS' + j + idx, 5,
						[{
							title: _('Siblings'),
...
function printIndiParents(fdx, idx, j)
// JV added idx,j params so can use them in here to find parent roles
{
	var html = '';
	...
	else
	{
		...
		// html+= '<p> JV - fdx is ' + fdx + '</p>';
		// html+= '<p> JV - j is ' + j + '</p>';
		// html+= '<p> JV - idx is ' + idx + '</p>';

		html += '<p> ' + _('Parental relationships to') + ' <i>' + I(idx, 'short_name') +'</i> - ';
		if (I(idx,'famc')[j])
		{
			html += I(idx,'famc')[j]['to_mother'] + ' ' + _('by this mother')  + ' ' +  ' , ';
			html += I(idx,'famc')[j]['to_father'] + ' ' + _('by this father')  + ' '   ;
		}
		else
		{
			html += _('not a child in this family (it is another marriage/union of a parent)');
		}
		html += '</p>';
	}
	return(html);
...
function printIndiDescendants(idx)
{
	...
	for (var j = 0; j < I(idx, 'fams').length; j++)
	{
		...
		html += PrintTitle('Iasc' +j + idx, 4, // JV fixed collapsing error
			[{
				// JV added marriage type to title
				title: famLinked(fdx)+ ' <small> ' + _('Union:') + ' ' + F(fdx, 'type') +'</small>',
...
function printIndiSpouses(fdx, idx)
{
	...
	{
		html += '<p class="dwr-ref">' + _('Spouse') + ' ' + indiLinked(spouses[k]) + '<br></p>';
	}
...
// modified JV  to use birth relationships function.
{
	var txt = '';
	txt += indiLinked(fc.index);
	txt += citaLinks(fc.cita);
	if (fc.note != '') txt += '<p><b>' + _('Notes') + ':</b></p>' + notePara(fc.note, '</p>');
	txt += printChildBirthReln(fc);
	return(txt);
}

function printChildBirthReln(fc)
// JV  print parent birth relationships on one line.  changed the span classes around to change bolding
{
	var txt = '';
	var txtf= '';
	var txtm = '';
	var relf, relm, titlef, titlem, nbspace;
	nbspace = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
	relf = fc.to_father;
	relm = fc.to_mother;
	titlef = _('Relationship to Father');
	titlem = _('Relationship to Mother');
	if (relf != '' && relf != _('Birth')) {
		txtf += '<span class="dwr-attr-value">' + titlef + ': </span>' + '<span class="dwr-attr-title">' + relf + '</span>' + nbspace;
	}
	if (relm != '' && relm != _('Birth')) {
		txtm += '<span class="dwr-attr-value">' + titlem + ': </span>' + '<span class="dwr-attr-title">' + relm + '</span>' + nbspace;
	}
	if ((txtf.length > 0) || (txtm.length > 0)) txt += '<p>' + nbspace + txtm + txtf + '</p>';
	return(txt);
}
...
function htmlPersonsIndexList(header, data)
{
	...
	var sortingAttributes = [
		...
		{
			title: _('Birth date - earliest first'), // JV changed sorts into by earliest and by latest
			id: 'I.birth_date_early',
			fSort: function(a, b) {return(I_birth_sdn[a] - I_birth_sdn[b])},
			fLetter: false
		},
		{
			title: _('Birth date - latest first'),
			id: 'I.birth_date',
			fSort: function(a, b) {return(I_birth_sdn[b] - I_birth_sdn[a])},
			fLetter: false
		},
		{
			title: _('Death date - earliest first'), // JV changed sorts into by earliest and by latest
			id: 'I.death_date_early',
			fSort: function(a, b) {return(I_death_sdn[a] - I_death_sdn[b])},
			fLetter: false
		},
		{
			title: _('Death date - latest first'),
			id: 'I.death_date',
...
function HomePage()
{
	...
	html += '<h1>' + TITLE + '</h1>';
	html += '<p>';
	html += '<p>' + _('Click a category to see a list') + '</p>';
	var tables = [
		["N", "surnames.html"],
        ["I", "persons.html"],
        ["F", "families.html"],
	...
	// JV display category list as table
	//JV added width, but width is obsolete, should do it in the CSS
	html += '<table style="width:50%" class="table table-condensed table-bordered dwr-table-flat">';
	html += '<tr><td class="dwr-attr-title">' + _('Category') + '</td></tr>';
	var sep = '';
	for (var i = 0; i < tables.length; i += 1)
	{
		var j = $.inArray(tables[i][1], PAGES_FILE_INDEX);
		if (j == -1) continue;
		html += '<tr><td  class="dwr-attr-value">';
		// html += sep;
	...
	// JV list the track relations
	if (TRACKING_TAG !== '')
	{
		html += '<br>';
		html += '<table class="table table-condensed table-bordered dwr-table-flat">';
		html += '<tr><td class="dwr-attr-title">' + _('Tracked relations') + '</td></tr>';
		if (TRACK_LIST.length > 0)
		{
			html += '<tr><td  class="dwr-attr-value">' + 'Relationship tracking will show connections, if any, to these individuals:' + '</td></tr>';
			var listItem = '';
			for (var i = 0; i < TRACK_LIST.length; i++)
			{
				listItem = TRACK_LIST[i];
				html += '<tr><td  class="dwr-attr-value">' + '<li> ' + indiLinked(listItem) + '</li></td></tr>';
			}
		}
		else
		{
			html += '<tr><td  class="dwr-attr-value">' + 'No individuals are tagged with: (' + TRACKING_TAG + ')'  + '</td></tr>';
		}
		html += '</table>';
	}
...
function MainRun()
{
	...
	try
	{
		...
		// if (Dwr.search.Idx >= 0 && ($.inArray(PageContents, [Dwr.PAGE_SVG_TREE, Dwr.PAGE_SVG_TREE_FULL, Dwr.PAGE_SVG_TREE_SAVE]) >= 0))
		{
		...
		// else if (PageContents == Dwr.PAGE_HOME)
		// {
		// 	html = HomePage();
		// }
		else
